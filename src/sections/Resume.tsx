...
/**
 * 履历：终端 console log 风格
 * 每个分组是一次 $ cat xxx.log，条目是 [序号] 时间段 标题 + └── 续行，
 * 底部一个闪烁光标。内容在 src/content/resume.ts 改
 */
export default function Resume() {
  return (
    <section id="resume" className="section-shell py-24">
      <SectionTitle eyebrow="RESUME" title="履历（占位，随时改）" />
      ...
      <div className="overflow-hidden rounded-xl border border-white/10 bg-surface">
        ...
        <div className="flex items-center gap-2 border-b border-white/10 px-5 py-3">
          ...
          <span className="h-2.5 w-2.5 rounded-full bg-white/15" />
          <span className="mx-auto font-mono text-xs text-muted">resume.log — zsh</span>
          <span className="w-10" />
        </div>
                  ...
                  >
                    ...
                    {entry.detail && (
                      <p className="mt-1 text-muted">
                        <span className="text-accent/60">└──</span> {entry.detail}
                      </p>
                    )}
                  </div>
                ))}
